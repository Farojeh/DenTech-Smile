...
part 'student_marks_state.dart';

class StudentMarksCubit extends Cubit<StudentMarksState> {
  final Dio dio = Dio();
  StudentMarksCubit() : super(StudentMarksInitial()){
    initialMarks();
  }

    final apiService = getIt<ApiService>();

  Future<void> initialMarks() async {
    try {
      emit(StudentMarksLoading());
      Response response;
      response =
          await apiService.get(endPoint: "/students-Marks", token: true);
      if (response.statusCode != 200 && response.statusCode != 201) {
        var failure =
            ServerFaliure.fromResponse(response.statusCode!, response.data);
        emit(StudentMarksFailure(errormessage: failure.errorMessage));
        return;
      }
      String path = response.data["download_url"];
      await downloadFile("${Static.urlimagewithoutstorage}$path", path);
      emit(StudentMarksSuccess(path: path));
    } catch (error) {
